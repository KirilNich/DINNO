...
function draw() {
    ...
    if (
        ...
        !world.objects.dino.element.src.endWith("dino/jump.gif")
    ){
        world.objects.dino.src = "dino/jump.gif"
    } else if (
            world.objects.dino.isJumping &&
            !world.objects.dino.element.src.endWith("dino/walk.gif")
    ){
...
function update(){
    ...
    world.objects.roadLines.forEach((roadLine)=> {
        ...
        if(roadElement.left + roadLineRect.width - 10 <= roadRect.left){
